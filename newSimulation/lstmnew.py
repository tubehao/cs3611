...
class Node(object):
    ...
    def __init__(self, node_id, m):
        ...
        self.access_history = {}
        self.cache = {}
        self.lstm_model = LSTMPredictor(input_size=2, hidden_size=50, output_size=1)
        self.criterion = nn.MSELoss()
        self.optimizer = torch.optim.Adam(self.lstm_model.parameters(), lr=0.001)

    def record_access(self, key):
        distance_to_key = self.distance(self.node_id, key)
        ...
        self.access_history[key].append([self.access_count[key], distance_to_key])

    def train_model(self, key):
        if len(self.access_history[key]) < 10:
            return  # Not enough data to train on
        history = np.array(self.access_history[key])
        sequence = torch.tensor(history[:-1], dtype=torch.float).view(-1, 1, 2)
        target = torch.tensor(history[1:, 0], dtype=torch.float).view(-1, 1, 1)
    ...
    def predict_access(self, key):
        ...
        with torch.no_grad():
            history = np.array(self.access_history[key])
            sequence = torch.tensor(history, dtype=torch.float).view(-1, 1, 2)
