...
class Network:
    ...
    def insert_nodes(self, nodes):
        for node in nodes:
            try:
                ...
                print(f'[+]Node {node.node_id} joined the network via node: {self.first_node.node_id}')
    ...
    def find_data(self, data):
        ...
        node = node.find_successor(hashed_key)[0]
        found_data = node.data.get(hashed_key, None)

        if found_data != None:
            print(f'[+]Found \'{data}\' in node {node.node_id} with key {hashed_key}')
        else:
    ...
    def insert_data(self, key):
        ...
        print(f'[+]Saving Key:{key} with Hash:{hashed_key} -> Node:{node.find_successor(hashed_key)[0].node_id}')
    ...
    def periodic_fix(self, interval=60):
        for node in self.nodes:
            node.fix_fingers()
